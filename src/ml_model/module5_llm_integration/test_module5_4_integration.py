...
def main() -> int:
    print("=" * 72)
    print("MODULE 5.4 REAL PROJECT INTEGRATION TEST")
    print("=" * 72)
    ...
    # ------------------------------------------------------------------
    # 7. MODULE 5.4 - CONFIDENCE SCORING
    # ------------------------------------------------------------------
    print("\n[7/8] MODULE 5.4 - evidence-aware confidence scoring")
    ...
    confidence_assessment = confidence_scorer.score(
        ...
        model_confidence=bedrock_response.confidence,
    )

    assert 0.0 <= confidence_assessment.score <= 1.0
    assert 0.0 <= confidence_assessment.agent_confidence <= 1.0
    assert 0.0 <= confidence_assessment.evidence_quality <= 1.0
    assert 0.0 <= confidence_assessment.agent_agreement <= 1.0
    assert confidence_assessment.model_confidence == bedrock_response.confidence
    assert confidence_assessment.metadata["agent_count"] == len(agent_results)
    assert confidence_assessment.metadata["unique_evidence_count"] == len(
        evidence_result.evidence_ids
    )

    assessment_id = confidence_scorer.stable_assessment_id(
        confidence_assessment
    )
    assert assessment_id.startswith("confidence:")
    assert len(assessment_id) == len("confidence:") + 16

    # Ground the recommendation-level confidence in the evidence-aware
    # pipeline score. The recommendation report already exposes an overall
    # confidence value, so combine that signal with the new assessment.
    recommendation_signal = {
        "confidence": recommendation_report.overall_confidence
    }
    grounded_recommendation_confidence = (
        confidence_scorer.recommendation_confidence(
            recommendation_signal,
            confidence_assessment,
        )
    )

    assert 0.0 <= grounded_recommendation_confidence <= 1.0

    print(
        "  Agent confidence      :",
        confidence_assessment.agent_confidence,
    )
    print(
        "  Evidence quality      :",
        confidence_assessment.evidence_quality,
    )
    print(
        "  Agent agreement       :",
        confidence_assessment.agent_agreement,
    )
    print(
        "  Model confidence      :",
        confidence_assessment.model_confidence,
    )
    print(
        "  Execution quality     :",
        confidence_assessment.execution_quality,
    )
    print(
        "  Final confidence      :",
    ...
    print("  [PASS] Module 5.4")

    # ------------------------------------------------------------------
    # 8. FINAL
    # ------------------------------------------------------------------
    print("\n[8/8] END-TO-END RESULT")

    print("  Module 1   -> PASS")
    print("  Module 2   -> PASS")
    print("  Module 3   -> PASS")
    print("  Module 4   -> PASS")
    print("  Module 5.2 -> PASS")
    print("  Module 5.3 -> PASS")
    print("  Module 5.4 -> PASS")

    print()
    print("=" * 72)
    print("MODULE 5.4 REAL PROJECT INTEGRATION TEST: PASSED")
    print("=" * 72)
